...
def plot_data_v1(gen_marker_group, dist_exp, data):
    f = plt.figure()
    ax = plt.gca()
...
def plot_coverage_version1(data, outfile, list_gen_markers_groups):
    ...
    for group in list_gen_markers_groups:
        plot_data_v1(group, dist_exp, data)
    ...
    for i in ax:
        ...
        try:
            ...
            ax[k].set_position([box.x0, box.y0, box.width * 0.92, box.height])
            #put legend on the right side
            legend = ax[k].legend(loc = 'center left', bbox_to_anchor = (1 , 0.5))
            frame = legend.get_frame()
        except AttributeError as e:
            # No legend, likely because no labels
            pass
        ...
        if outfile:
            plt.savefig(outfile + "_" + str(k), dpi=300)
        plt.show()
        k = k + 1
...
def plot_coverage_version2(data, outfile):
    ...
    for exp in dist_exp:
        ...
        if msft['True case'].iloc[0] == 'P':
            ...
            #marker = next(markerP))
            sns.stripplot(msft['Gen marker'], msft['mean'], color = 'g', label = exp,
                            marker = next(markerN), s = 11, jitter = 0.05)
        else:
            #ax.scatter(msft['Experiment'], msft['mean'], color = next(color), label = msft['Gen marker'],
            #marker = next(markerN))
            sns.stripplot(msft['Gen marker'], msft['mean'], color = 'r', label = exp,
                            marker = next(markerN), s = 11, jitter = 0.05)
