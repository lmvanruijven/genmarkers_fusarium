    ...
    while blast_hits:
        try:
            ...
            if len(blast_hits[BLASTstart+3]) < 16:
                ...
                #remove \n from query id value
                #df_hits['query id'] = df_hits['query id'].map(lambda x: x.rstrip('\n'))
                #print('df_hits' , df_hits['query id'][0])
                temp_key = blast_hits[BLASTstart+1].replace('# Query: ', "").strip('\n')
...
#results = data.join(data.groupby(['gen marker','experiment', 'input data'])['hits'].mean().reset_index().rename(columns={0: '%'}) ,
    ...
    #print(results)
    #print(var_perc_pathogen)

    #list_agg = [ 'mean', 'min', 'max', 'std']
    #
    #data = data.groupby( ['gen marker','input data'], as_index = False).agg({'% pathogen': list_agg})
    #print(data)
    return results
...
def results_plot(data, output, gen_marker_group):
    ...
    #print(data)
    #data = data[(data['experiment'].str.split('+').str[0] == 'B')]
    ...
    ax.axis('tight')
    #
    table = ax.table(cellText = data.values, colLabels = data.columns) #, colWidths = [0.1] * len(data.columns) )
